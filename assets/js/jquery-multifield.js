...
(function ($) {
    $.fn.EnableMultiField = function (options) {
        options = $.extend({
            linkText: '<img src="../assets/images/add.png"/>',
            linkClass: 'addMoreFields',
            enableRemove: true,
            removeLinkText: '<img src="../assets/images/remove.png"/>',
                ...
                if (options.maxItemsAllowedToAdd === null || totalCount < options.maxItemsAllowedToAdd) {
                    ...
                    $(newElem).find("*[id!=''][name!='']").each(function () {
                        if ($(this).attr("id")) {
                            ...
                            if( class_name == "text-red error_message plot_number_error_msg error" ){
								
				var class_val = "text-red error_message plot_number"+fieldCount+"_error_msg error";
								$(this).attr("class", class_val);
								
					}else if( class_name == "text-red error_message phase_name_error_msg error"){
						
				var class_val = "text-red error_message phase_name"+fieldCount+"_error_msg error";
								$(this).attr("class", class_val);
								
					}else if( class_name == "text-red error_message site_id_error_msg error"){
						
				var class_val = "text-red error_message site_id"+fieldCount+"_error_msg error";
								$(this).attr("class", class_val);	
								
					}else if(class_name == "text-red error_message plot_size_id_error_msg error"){
				
				var class_val = "text-red error_message plot_size_id"+fieldCount+"_error_msg error";
								$(this).attr("class", class_val);	
						
					}else if(class_name == "text-red error_message site_face_id_error_msg error"){
				
				var class_val = "text-red error_message site_face_id"+fieldCount+"_error_msg error";
								$(this).attr("class", class_val );	
						
					}else if(class_name == "sequence"){
						
							var class_val = fieldCount;
							$(this).attr("value", class_val);
						
					}
													
								
								
							//$(this).attr("name", class_val + "$" + fieldCount);
                        }
                    });
